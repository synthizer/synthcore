...
using TestModSlice = synthcore::ModSlice<int, synthcore::mod_pointer_detail::StaticModProvider<LEN>>;
...
TEST_CASE("ModSlice") {
  ...
  SECTION("basic indexing") {
    auto ptr =
        TestModSlice<NUM_INDICES>{data.data(), 5000, synthcore::mod_pointer_detail::StaticModProvider<NUM_INDICES>{}};
    REQUIRE(*ptr == 5000);
    REQUIRE(ptr[0] == 5000);
  ...
  SECTION("Basic incrementing loops") {
    auto ptr =
        TestModSlice<NUM_INDICES>{data.data(), 0, synthcore::mod_pointer_detail::StaticModProvider<NUM_INDICES>{}};
...
TEST_CASE("ModPointer") {
  ...
  auto ptr = synthcore::createStaticModPointer<int, 5>(data, offset, max_index);
  REQUIRE(ptr.index() == variant_index);

  auto ptr2 = synthcore::createDynamicModPointer<int>(data, offset, max_index, 5);
  REQUIRE(ptr.index() == variant_index);
}
