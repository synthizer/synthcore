...
enum class NoiseType {
  ...
  /**
   * Use the Voss-mcCartney algorithm to generate pink noise.
   * */
  PinkVm,

  /**
   * Generate brown noise via a filter.
   * */
  FilteredBrown,
};
...
private:
  ...
  NoiseType noise_type = NoiseType::Uniform;

  /**
   * Implement the Voss-mcCartney algorithm to generate pink noise.
   *
...
inline NoiseType NoiseGenerator::getNoiseType() { return this->noise_type; }

inline void NoiseGenerator::setNoiseType(NoiseType type) {
  this->noise_type = type;
  if (this->noise_type == NoiseType::PinkVm) {
    this->initVM();
  } else if (this->noise_type == NoiseType::FilteredBrown) {
