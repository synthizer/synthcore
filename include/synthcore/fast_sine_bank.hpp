...
public:
  ...
  /**
   *  Frequency, as a multiplier of the base frequency. E.g 1.0 is the base
   *  frequency, 2.0 the first harmonic.
   * */
  double freq_mul;
  double gain;
};
 ...
 * ```
 *
 * By holding b constant, we can do the entire thing without any actual trig
 * functions after initialization.  That is a is the advancing/output angle.
 *
 * This uses templates for autovectorization of computation, primarily (but not
 * only) for multiples of 4.  To keep accuracy, it resynchronizes each time
 * `fillBlock` is called.
 *
 * SR is hardcoded at the synthcore samplerate, as per usual.
 * */
class FastSineBank {
public:
...
private:
  ...
  // To allow for frequency changes without artifacts or fading, we must store
  // time on a per-wave basis.
  class WaveState {
  public:
