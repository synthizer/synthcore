...
/**
 * An EdgeTrigger provides trigger notifications when a boolean value switches
 * from false to true, true to false, or both.  This is named for and functions
 * like the electronics equivalent.
 *
 * To use, create an instance of EdgeTrigger, then call evaluate once a block
 * (or on whatever interval is appropriate).
 *
 * This is used as part of the event system, which generally needs to fire
 * events when a condition changes.
 *
 * It is permissible for the callbacks here to have side effects.  This
 * interface isn't intended to be pure, only to get a particular coding idiom
 * out of the way of more important logic.  In particular the condition is
 * called on every call to evaluate.
 * */
...
/**
 * The type-erased interface and functions.  Though synthcore tries to allocate
 * inline this is one place where a convenient interface requires allocating on
 * the heap.
 *
 * This probably isn't the class you want to use; see EdgeTrigger, below, which
 * provides an abstraction that hides the heap allocations,.
 * */
...
/**
 * An edge trigger. Pass two callbacks to the constructor, one to evaluate the
 * condition and one to call when the trigger fires, then call evaluate
 * periodically.
 * */
class EdgeTrigger {
public:
