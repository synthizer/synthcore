...
inline IIRFilterDef<1, 2> designOnePole(double pole) {
  ...
  // -1, because a1 > 0, giving 1/(1-a0) 1, because a1 < 0, giving
  // 1/(1-abs(a0)) The former expression can have abs added without changing
  // its value, so we avoid the conditional.
  ret.gain = 1 - abs(a1);
  return ret;
}
...
/**
 * Implement the cases from the audio EQ cookbook, in the root of this
 * repository.
 *
 * Instead of frequency we use omega, which is frequency/sr. This is because not
 * all of the library operates at config::SR (i.e. oversampled effects).
 *
 * For lowpass and highpass, default q gives butterworth polynomials in the
 * denominator.
 * */
 ...
 *
 * For the time being must be an odd length.
 * */
template <std::size_t N> auto designSincLowpass(double omega) -> IIRFilterDef<N, 1> {
  ...
  for (unsigned int i = 0; i < coefs.size(); i++)
    gain += coefs[i];
  // add a little bit to the denominator to avoid divide by zero, at the cost of
  // slight gain loss for small filters.
  gain = 1.0 / (gain + 0.01);
