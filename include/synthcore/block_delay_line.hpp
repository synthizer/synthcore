...
/*
 * This file implements a delay line which allows for delays of up to N blocks,
 * and contains special helper methods to avoid modulus in some cases. The data
 * is allocated statically.
 *
 * Primarily, this is of use to the HRTF implementation which uses
 * BlockDelayLine to avoid modulus on most blocks, and will probably also be
 * used by FDN reverberation for similar reasons
 *
 * This is opaque, but there are two usage patterns:
 *
 * The per-block method:
 * - Something calls line.getNextBlock(), which returns a pointer to the next
 *   block of data.
 * - Something fills the next block of data.
 * - Then call `getModPointer` to read data.
 *
 * The per-sample pattern:
 * - Something calls runRWLoop with a closure which gets passed something with
 *   read as above, but also write(unsigned int channel, float value).
 *
 * In the first pattern, and in the case that the closure writes before reading
 * in the second pattern, 0 delay is possible.
 * */
