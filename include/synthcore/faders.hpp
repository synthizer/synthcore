 ...
 * once they've done so, they'll return the exact output value specified.  This
 * is used by the router to avoid having to maintain an FSM
 *
 * Faders are configured once at construction, then reconfigured by
 * reconstructing as necessary. This is done because synthcore needs to be well
 * behaved in the presence of rapid user changes.
 * */
...
public:
  LinearFader(unsigned int start_time, float start_value, unsigned int end_time, float end_value)
    ...
    /**
     * Lots of places make faders that don't fade by using start_time = end_time
     * = 0, which causes MSVC warning C4723, due to a genertaed divide by zero.
     * Branch, detecting this case, to make sure that MSVC doesn't complain.
     * */
